...
const ChatModal = ({ show, onHide, sessionData }) => {
    ...
    const handleSend = async (e) => {
        ...
        try {
            ...
            const errorMessage = { role: 'assistant', content: "Sorry, I couldn't process that. Please try again." };
    ...
    return (
        <Modal show={show} onHide={onHide} size="lg" centered>
            <Modal.Header>
                ...
                <Button variant="light" onClick={onHide} className="rounded-circle p-1">
                    <FaTimes />
                </Button>
            </Modal.Header>
            <Modal.Body className="p-0" style={{ height: '60vh', display: 'flex', flexDirection: 'column' }}>
                <div className="chat-area p-3 overflow-auto flex-grow-1" style={{ backgroundColor: '#f9f9f9' }}>
                    {messages.length === 0 && !isLoading ? (
                        ...
                        messages.map((msg, index) => (
                            <div key={index} className={`d-flex ${msg.role === 'user' ? 'justify-content-end' : 'justify-content-start'} mb-2`}>
                                <div 
                                    className={`p-2 rounded-3 ${msg.role === 'user' ? 'bg-primary text-white' : 'bg-light border'}`}
                                    style={{ maxWidth: '75%' }}
                                >
                                    {msg.content}
                                </div>
                    ...
                    {isLoading && (
                        <div className="d-flex justify-content-start mb-2">
                            <div className="p-2 rounded-3 bg-light border">
                                <Spinner animation="grow" size="sm" variant="secondary" className="me-1" />
                                Typing...
                            </div>
                        </div>
                    )}
                ...
                <Form onSubmit={handleSend} className="p-3 border-top">
                    <div className="input-group">
                        <Form.Control
                            ...
                            disabled={isLoading}
                        />
                        <Button type="submit" variant="warning" disabled={!input.trim() || isLoading}>
